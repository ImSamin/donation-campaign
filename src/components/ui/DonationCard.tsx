    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {displayedData.map((item : any) => (
          <div key={item.id} className="bg-blue-600 bg-opacity-25 rounded-lg overflow-hidden shadow-md flex">
            {/* Left Side */}
            <div className="w-1/3">
              <Image src={cardImage} alt="Photo 1" className="w-full h-full" />
            </div>
            {/* Right Side */}
            <div className="p-4 px-5 w-2/3">
              ...
              <h2 className="mt-2  text-black text-xl font-semibold">{item.title}</h2>
      ...
      {!showAll && (
        <div className="mx-auto justify-center mt-10 w-20">
          <Button className="bg-green-700" type="primary" onClick={handleSeeAll}>
            See All
          </Button>
        </div>
      )}
